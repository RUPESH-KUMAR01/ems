...
public class EventTeamRequestServiceRigorousConcurrencyTest {
    ...
    public void setUp() {
        ...
        teamService = new EventTeamService(eventDao, teamDao, teamMemberDao, requestDao, eventRegistrationDao);
        requestService = new EventTeamRequestService(userDao, requestDao, teamService, eventRegistrationService);
    }
    ...
    public void testPessimisticLockPreventsTeamOverfill() throws InterruptedException {
        ...
        Long ownerId = daoTestRule.inTransaction(() -> {
            User user = new User();
        ...
        Long eventId = daoTestRule.inTransaction(() -> {
        ...
        Long teamId = daoTestRule.inTransaction(() -> {
        ...
        daoTestRule.inTransaction(() -> {
            for (int i = 0; i < 10; i++) {
                ...
                TeamMembershipRequest req = new TeamMembershipRequest(teamId, user.getId(), RequestType.JOIN_REQUEST);
        ...
        // Assuming id isn't explicitly settable here, we'll construct Principal with full constructor if we could, 
        // but UserPrincipal has constructor UserPrincipal(Long, String, String, String, Role, boolean, boolean)
        UserPrincipal ownerPrincipal = new UserPrincipal(ownerId, "owner@example.com", "1234567890", "Owner", Role.USER, true, true);
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            futures.add(executor.submit(() -> {
                ...
                try {
                    ...
                    requestService.respondToRequest(eventId, teamId, userId, response, ownerPrincipal);
                    tx.commit();
                    success = true;
                } catch (Exception e) {
                    tx.rollback();
